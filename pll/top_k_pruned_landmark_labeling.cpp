  ...
  for (int dir = 0; dir < 1 + directed; dir++){
    for (size_t v = 0; v <  V; v++){
      ...
      for (size_t i = 0; idv.label[i] != V;){
        ...
        size_t ni = i;
        size_t offset = 0;
        size_t max_ni = std::min(i + dist_array_t::size, idv.length);
        while (ni < max_ni && idv.label[ni] != V) {
          size_t j = 0;
          do {
  ...
  for (int dir = 0; dir < 1 + directed; dir++){
    ...
    for (size_t v = 0; v < V; v++){
      ...
      for (size_t i = 0; idv.label[i] != V;){
        ...
        d_count.reserve(MAX_EXPECTED_SIZE);     
        size_t ni = i;
        size_t max_ni = std::min(i + dist_array_t::size, idv.length);
        while (ni < max_ni && idv.label[ni] != V) {
          uint8_t count = 0;
...
// Add these extern "C" declarations at the end of the file

extern "C" {
    TopKPrunedLandmarkLabeling* create_pll() {
        return new TopKPrunedLandmarkLabeling();
    }

    void free_pll(TopKPrunedLandmarkLabeling* ptr) {
        delete ptr;
    }

    bool construct_index(TopKPrunedLandmarkLabeling* pll, 
                        int* edges_a, int* edges_b, int num_edges,
                        int K, bool directed) {
        std::vector<std::pair<int, int>> edges;
        for(int i=0; i<num_edges; i++) {
            edges.emplace_back(edges_a[i], edges_b[i]);
        }
        return pll->ConstructIndex(edges, K, directed);
    }

    int k_distance_query(TopKPrunedLandmarkLabeling* pll, 
                        int s, int t, int k, int* results) {
        std::vector<int> dists;
        int ret = pll->KDistanceQuery(s, t, k, dists);
        if(ret == 0) {
    ...
    bool store_index(TopKPrunedLandmarkLabeling* pll, const char* path) {
        return pll->StoreIndex(path);
    }

    bool load_index(TopKPrunedLandmarkLabeling* pll, const char* path) {
        return pll->LoadIndex(path);
    }
}